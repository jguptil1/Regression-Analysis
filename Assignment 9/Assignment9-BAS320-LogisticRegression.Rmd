...
I'm using a logistic regression model to predict the probability that ... (specify your Y variable in plain English and the level that your model is predicting, don't necessarily use the column name in the data) from ... (describe the X variables you're using in plain English)
...
The data I'm using comes from ... (if kaggle, tell us the URL; if the BAS320datasetsYCategoricalFall23.RData  file you can just tell us that) and contains a total of ... rows and ... total predictors (always important to tell us how much data).


## Task 1 - Investigation of the relationship between the probability of ... (replace with what you're studying) and ... (replace with a numeric X)
...
(Write a short sentence that talks about whether individuals with larger values of X have a larger or smaller probability that individuals with smaller values of X.  Write a short sentence that tells us the value of X where there's a 50% chance of observing the level of interest.  If this value of X is far outside the range of X used to the build the model, that would be extrapolating, and make a note that this statement is a "guess" at best because of that)
...
## Task 2 - Investigation of the relationship between the probability of ... (replace with what you're studying) and ... (replace with a numeric X1 and categorical X2)
...
## Task 3 - Investigation of the relationship between ... (replace with Y) and all predictors (full model)
